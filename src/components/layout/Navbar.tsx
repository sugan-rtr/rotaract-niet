...
export default function Navbar() {
  ...
  return (
    ...
    >
      <nav
        className={`pointer-events-auto flex items-center justify-between w-full rounded-full transition-all duration-500 ease-[cubic-bezier(0.16,1,0.3,1)] ${scrolled || open
      ...
      >
        <div className="flex items-center justify-start lg:w-[180px] xl:w-[200px] shrink-0">
          ...
          >
            ...
            <img src="/images/logo.png" alt="Rotaract Club of NIET Logo" className="w-8 h-8 rounded-full shadow-[0_0_10px_var(--color-accent)] shrink-0 transition-transform duration-300 group-hover:scale-110" />
            <span>Rotaract&nbsp;Club of NIET</span>
          </a>
        </div>
            ...
            {navLinks.map((link) => {
              ...
              return (
                <li key={link.href}>
                  <a
                    href={link.href}
                    className={`relative rounded-full px-3 py-1.5 text-[13px] xl:text-sm font-medium tracking-wide transition-all duration-300 block whitespace-nowrap border ${isActive
                ...
                return (
                  <li key={link.href}>
                    <a
                      ...
                      className={`flex items-center justify-between rounded-2xl px-4 py-3 text-sm font-medium transition-all duration-300 border ${isActive
